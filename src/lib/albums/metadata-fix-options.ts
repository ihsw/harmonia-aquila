...
function parseProducerStrategy(value: string | undefined): ProducerStrategy {
  ...
  if (!['no change', 'blank', 'aggregate', 'copy-from-album-artists'].includes(strategy)) {
    throw new UserInputError(
      '--producer-strategy must be one of: no change, blank, aggregate, copy-from-album-artists',
    )
  }
  return strategy as ProducerStrategy
}

function validateOptionConflicts(options: NormalizedMetadataFixOptions): void {
  ...
  validateSetMetadataConflicts(options)
}
...
export function normalizeMetadataFixOptions(options: MetadataFixOptions): NormalizedMetadataFixOptions {
  const normalized: NormalizedMetadataFixOptions = {
