...
export function registerOrganizeFilesCommand(program: Command): void {
  const organizeFilesCommand = program
    ...
    .option('--source-dirs <sourceDirs...>', 'ordered flat album directories to concatenate into one album output')
    ...
    .option('--disc-strategy <strategy>', 'how to update disc metadata: no change, infer, concatenate', 'no change')
