...
- [ ] Inventory sequential `.mp3` and `.m4b` folders and save ordered source
      paths plus artist/album/title metadata in `inventory.json`.
- [ ] Confirm the 35 `The Wealth and Poverty of Nations A/` parts still lack
...
- [ ] Confirm each candidate has at least two audio parts with one complete,
      shared performer and album/title identity (FR-2).
- [ ] Exclude every missing, inconsistent, or multi-group candidate unless it
...
- [ ] For each eligible folder, run the `merge --jobs 16 --format json` dry
      run; add `--bypass-metadata` only for explicitly approved source folders.
- [ ] Confirm the staged destination is absent and the dry-run source count matches
      the inventory before adding `--execute`.
- [ ] Execute the approved merge with `--jobs 16`, saving its JSON result;
      stop on any failure.
...
- [ ] Dry-run `set-metadata` from the staged M4B to the exact final path with
...
- [ ] Confirm the final path is absent, then rerun the exact command with
...
- [ ] Validate every created M4B and save `validation.json`.
- [ ] Confirm all source files and directories remain present and unchanged.
- [ ] Run `npm run build` and `npm run lint`; both must exit 0.
- [ ] Confirm `git --no-pager diff --stat -- src package.json package-lock.json`
      is empty.
...
- [ ] Retain inventory, dry-run, execution, metadata, and validation JSON records under
