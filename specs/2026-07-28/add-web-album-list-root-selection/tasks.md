...
- [x] Inspect dirty-worktree state and preserve the unrelated untracked `.ai/`
      directory.
- [x] Reconfirm current REST query parsing, GraphQL input/SDL generation, MCP
      registration, and `useScratchDir` conventions from `design.md`.
- [x] Run the three focused Vitest commands from `design.md` section 9 and
...
- [x] Extend `__tests__/web/controllers.test.ts` for omitted/false source,
      true scratch, scratch-plus-prefix, and invalid/repeated selector cases.
- [x] Run `npm run lint -- __tests__/web/controllers.test.ts` after every edit
      and fix all reported issues.

### 2.2 Implement REST root selection

- [x] Add optional query-boolean parsing to
      `src/web/schemas/request-schemas.ts` (FR-1, FR-6).
- [x] Run `npm run lint -- src/web/schemas/request-schemas.ts` after every
      edit and fix all reported issues.
- [x] Update `ManageAlbumsController.list` with exact-true root selection and
...
- [x] Run the focused controller test until it exits 0.

## Phase 3 — GraphQL adapter

### 3.1 Specify GraphQL selector behavior

- [x] Extend `__tests__/web/graphql/album.resolver.test.ts` for omitted/false
      source, true scratch, and scratch-plus-prefix mapping.
- [x] Extend `__tests__/web/graphql/graphql.integration.test.ts` for Boolean
      schema acceptance and invalid-type validation.
- [x] Run per-file `npm run lint -- <modified-file>` after every test edit.

### 3.2 Implement and generate the GraphQL contract

- [x] Add nullable `useScratchDir` Boolean to `AlbumListInput` (FR-2).
- [x] Run `npm run lint -- src/web/modules/graphql/album.inputs.ts` after every
      edit and fix all reported issues.
- [x] Update `AlbumResolver.albumList` with exact-true root selection.
- [x] Run `npm run lint -- src/web/modules/graphql/album.resolver.ts` after
      every edit and fix all reported issues.
- [x] Initialize the application to regenerate
      `src/web/modules/graphql/schema.gql`; do not hand-edit generated SDL.
- [x] Run the focused GraphQL tests until they exit 0.

## Phase 4 — MCP adapter

### 4.1 Specify MCP schema, selection, and metadata

- [x] Extend `__tests__/web/mcp.manage-albums.test.ts` for omitted/false
      source, true scratch, scratch-plus-prefix, invalid type, Boolean discovery
      schema, and retained read-only annotation.
- [x] Run `npm run lint -- __tests__/web/mcp.manage-albums.test.ts` after every
      edit and fix all reported issues.

### 4.2 Implement MCP root selection

- [x] Add optional boolean `useScratchDir` to
      `manageAlbumsListInputSchema` (FR-3, FR-7).
- [x] Run `npm run lint -- src/web/schemas/mcp/manage-albums.ts` after every
      edit and fix all reported issues.
- [x] Update the list handler's exact-true selection and discovery
      title/description (FR-4, FR-10).
- [x] Run `npm run lint --
      src/web/servers/mcp-tools/manage-albums/list.ts` after every edit and fix
      all reported issues.
- [x] Run the focused MCP test until it exits 0.

## Phase 5 — Bruno and documentation

### 5.1 Add live source/scratch coverage

- [x] Add `scratchAlbumEntry: scratch-only/` to the local Bruno environment.
- [x] Add REST and GraphQL scratch-list requests that pass
      `useScratchDir: true` and assert the scratch-only marker.
- [x] Add MCP default-source and scratch-selected list calls; parse JSON tool
      content and assert the scratch-only marker on the latter.
- [x] Confirm the new requests invoke only the read-only list operation and
      retain the existing source-default requests.

### 5.2 Document the public selector

- [x] Update `docs/graphql.md` with omitted/false source behavior, a scratch
      query example, and the no-client-root boundary.
- [x] Update `docs/mcp-server.md` with `useScratchDir` semantics and revised
      list-tool description.
- [x] Update `docs/testing.md` to create the scratch-only marker required by
...
- [x] Create a temporary scratch directory with `mktemp -d` and its known
      `scratch-only/` marker.
- [x] Start `npm run web:serve` against the example source/destination roots
      and captured scratch root; retain the exact server PID.
- [x] Run the complete Bruno collection command from `design.md` section 9;
      REST, GraphQL, and MCP source/scratch requests MUST pass.
- [x] Stop the captured server PID, remove the known marker with `rmdir`, and
      remove the known empty scratch root with `rmdir`.

### 6.3 Scope and behavior audit

- [x] Run both forbidden-path diff commands from `design.md` section 9 and
      confirm they are empty (NFR-7, NFR-8).
- [x] Confirm the final diff contains only `design.md` section 2 files plus
      this spec directory.
- [x] Confirm existing prefix/error/response tests still pass, MCP remains
