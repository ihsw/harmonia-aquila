...
- [x] Check `git --no-pager status --short` and avoid reverting unrelated user
      changes.
- [x] Run `npm run test:coverage` and record the global statement, line,
      function, and branch percentages in a task note.
- [x] Confirm `reports/coverage/coverage-summary.json` exists and identifies
...
> Baseline: statements 61.09%, lines 61.38%, functions 72.58%, branches 49.80%.

### 1.2 Classify low-coverage files

- [x] For each Priority 1 file in `design.md` §2, decide whether it receives
      tests or is documented as a thin entrypoint/wrapper exclusion.
- [x] Record any exclusions as a blockquoted task note with rationale.

> Exclusion: `src/index.ts` remains an executable entrypoint that parses the
> real process command line; command registration coverage is provided through
> the index registration smoke tests instead.

## Phase 2 — Priority 1 domain-boundary tests

### 2.1 Cover album audio tag helpers

- [x] Add `__tests__/lib/albums/audio-tags.test.ts`.
- [x] Mock `music-metadata` and `node-taglib-sharp` at the module boundary.
- [x] Cover successful metadata reads, missing/invalid metadata branches, and
      tag write calls/errors without touching real media files.
- [x] Run `npm run lint -- __tests__/lib/albums/audio-tags.test.ts`. Fix
      issues. Re-run until clean.

### 2.2 Cover audiobook m4b-tool library wrapper

- [x] Add `__tests__/lib/audiobooks/m4b-tool.test.ts`.
- [x] Mock process execution so tests never invoke Docker or `m4b-tool`.
- [x] Cover command construction, success parsing, and failure propagation.
- [x] Run `npm run lint -- __tests__/lib/audiobooks/m4b-tool.test.ts`. Fix
      issues. Re-run until clean.

### 2.3 Cover album set-metadata command helper

- [x] Add `__tests__/commands/manage-albums/helpers/set-metadata.test.ts`.
- [x] Mock native tag writes and filesystem boundaries.
- [x] Cover option validation, dry-run/planning behavior, and write/error
...
- [x] Add tests for `src/commands/manage-audiobooks/helpers/audiobook-file.ts`
      and `src/commands/manage-audiobooks/helpers/m4b-tool.ts`, or document them
      as pure-wrapper exclusions.
- [x] If tests are added, run `npm run lint -- <modified-test-file>` for each
      file. Fix issues. Re-run until clean.

### 3.2 Cover command index registration

- [x] Add command registration smoke tests for `manage-albums`, `manage-audiobooks`,
      and `web` command indexes, or document any entrypoint exclusions.
- [x] Assert subcommand names/options without parsing real process arguments.
- [x] Run `npm run lint -- <modified-test-file>` for each modified test file.
      Fix issues. Re-run until clean.
...
- [x] Extend or add tests for `src/lib/audiobooks/set-metadata.ts`.
- [x] Extend or add tests for `src/lib/audiobooks/convert-file.ts`.
- [x] Cover error propagation, destination handling, and option/concurrency
...
- [x] Extend or add tests for `src/lib/albums/fix-tags.ts`.
- [x] Add tests for `src/lib/errors.ts`.
- [x] Cover strategy/error branches that currently pull down branch coverage.
- [x] Run `npm run lint -- <modified-test-file>` for each modified test file.
      Fix issues. Re-run until clean.

### 4.3 Cover command crawl branch gaps

- [x] Extend `__tests__/commands/manage-audiobooks/crawl.test.ts` or add a
      focused companion test for command-boundary branch gaps.
- [x] Run `npm run lint -- __tests__/commands/manage-audiobooks/crawl.test.ts`
...
- [x] Run `npm run test:coverage` and compare all four global metrics against
      the Phase 1 baseline.
- [x] Confirm statement, line, function, and branch coverage all improved.
- [x] Confirm whether global line coverage reached 70% and branch coverage
      reached 60%; document blockers if not.

> Final measured before thresholds: statements 86.30%, lines 86.44%,
> functions 94.75%, branches 70.78%.

### 5.2 Optional threshold update

- [x] Decide whether to add conservative global thresholds to `vitest.config.ts`.
- [x] If thresholds are added, keep them no higher than final measured coverage.
- [x] If `vitest.config.ts` is modified, run `npm run lint -- vitest.config.ts`.
      Fix issues. Re-run until clean.
...
- [x] If thresholds or new coverage conventions are added, update
      `docs/testing.md`.
- [x] If `docs/testing.md` is unchanged, document that no docs update was
...
- [x] `npm test` — exit 0.
- [x] `npm run test:coverage` — exit 0 and metrics improved versus baseline.
...
- [x] `git --no-pager diff --stat -- __tests__ src vitest.config.ts docs/testing.md specs/2026-07-19/raise-test-coverage/tasks.md` — output MUST list only expected test, approved seam, optional config/doc, and task files.
