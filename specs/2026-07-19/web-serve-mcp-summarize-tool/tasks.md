...
- [ ] Do **not** run whole-codebase `npm run lint` as a pre-flight baseline;
      ...
      complete.
- [ ] Inspect `package.json` and `package-lock.json` to confirm whether
      `@modelcontextprotocol/sdk` is already installed.
- [ ] Inspect `src/web/app.module.ts`, `src/web/main.ts`,
      `src/web/manage-albums.controller.ts`, `src/web/path-resolver.ts`, and
      `src/web/request-schemas.ts`.
- [ ] Inspect `collections/harmonia-aquila-web/**` and `docs/testing.md` for
      existing Bruno conventions.
- [ ] Run `./node_modules/.bin/vitest run __tests__/web` and capture the
...
- [ ] If `@modelcontextprotocol/sdk` is absent, run
      `npm install @modelcontextprotocol/sdk` to update `package.json` and
      `package-lock.json`.
- [ ] If the SDK is already present, do not modify package metadata.
- [ ] Confirm no other dependency was added.

### 2.2 Add MCP schemas

- [ ] Create `src/web/mcp-schemas.ts` with typed Zod schemas for
      `manage_albums_summarize_source_dir` input and any protocol-facing helper
      types needed by the controller.
- [ ] Ensure `limit` is accepted as a non-negative integer and transformed to
      the existing domain string option only in the handler layer.
- [ ] Run `npm run lint -- src/web/mcp-schemas.ts`. Fix issues. Re-run until
      clean.

### 2.3 Add MCP tool/server factory

- [ ] Create `src/web/mcp-server.ts` to register exactly one MCP tool named
      `manage_albums_summarize_source_dir`.
- [ ] Wire the handler to `WebPathResolver.resolveSource` and
      `summarizeAlbumSourceDir`.
- [ ] Ensure invalid input and path traversal errors are returned through the
      chosen MCP error shape without invoking the domain function.
- [ ] Run `npm run lint -- src/web/mcp-server.ts`. Fix issues. Re-run until
      clean.
...
- [ ] Create `src/web/mcp.controller.ts` with POST handling for MCP messages and
      GET behavior that returns 405 unless SSE support is explicitly
      implemented.
- [ ] Add local-origin validation before protocol handling.
- [ ] Run `npm run lint -- src/web/mcp.controller.ts`. Fix issues. Re-run until
      clean.

### 3.2 Register the controller

- [ ] Update `src/web/app.module.ts` so `createAppModule` registers the MCP
      controller and any web-scoped MCP provider.
- [ ] Run `npm run lint -- src/web/app.module.ts`. Fix issues. Re-run until
      clean.
...
- [ ] Create `__tests__/web/mcp.controller.test.ts` or equivalent focused web
      tests for initialize/tool listing, successful tool call, invalid input,
      traversal rejection, and unsafe Origin rejection.
- [ ] Assert `tools/list` exposes exactly one tool:
      `manage_albums_summarize_source_dir`.
- [ ] Assert traversal and invalid input do not call `summarizeAlbumSourceDir`.
- [ ] Run `npm run lint -- __tests__/web/mcp.controller.test.ts`. Fix issues.
      Re-run until clean.

### 4.2 Update bootstrap tests only if useful

- [ ] If route-level bootstrap coverage is added, update
      `__tests__/web/bootstrap.test.ts` without duplicating all MCP behavior.
- [ ] If modified, run `npm run lint -- __tests__/web/bootstrap.test.ts`. Fix
...
- [ ] Update `collections/harmonia-aquila-web/environments/local.yml` with MCP
      variables such as `mcpProtocolVersion`, `mcpRequestId`, and any
      session variable needed by the implemented SDK flow.
- [ ] Keep existing REST variables intact.

### 5.2 Add MCP requests

- [ ] Add `collections/harmonia-aquila-web/mcp/initialize.yml`.
- [ ] Add `collections/harmonia-aquila-web/mcp/initialized.yml` only if the
      implemented MCP session flow requires the initialized notification.
- [ ] Add `collections/harmonia-aquila-web/mcp/tools-list.yml`.
- [ ] Add
...
- [ ] Assert status codes, JSON-RPC ids, exact tool count/name, successful tool
...
- [ ] Update `docs/mcp-server.md` to distinguish this scoped `web serve` MCP
      endpoint from the broader future stdio MCP server, if the current doc
      would otherwise be misleading.
- [ ] Update `docs/testing.md` with MCP Bruno smoke instructions if the existing
...
- [ ] `cd collections/harmonia-aquila-web && ../../node_modules/.bin/bru run . -r --env local --bail`
      - exit 0.
...
- [ ] `npm test` - exit 0.
- [ ] `git --no-pager diff --stat -- package.json package-lock.json src/web __tests__/web collections/harmonia-aquila-web docs/mcp-server.md docs/testing.md`
